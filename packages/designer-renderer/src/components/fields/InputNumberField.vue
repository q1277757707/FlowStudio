...
<template>
  <FieldFormItem :mode="props.mode" :node="node">
    <el-input-number
      :model-value="modelValue(runtime.readNumber(node, 'defaultValue', 0)) as number"
      :min="runtime.readNumber(node, 'min', 0)"
      :max="runtime.readNumber(node, 'max', 100)"
      :step="runtime.readNumber(node, 'step', 1)"
